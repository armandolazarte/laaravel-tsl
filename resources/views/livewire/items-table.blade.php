...
<h1 class="text-2xl font-semibold text-gray-900">Items</h1>
        ...
        <div class="flex-col space-y-4">
            ...
            <x-table>
                <x-slot name="head">
                    ...
                    <x-table.heading sortable multi-column wire:click="sortBy('company_name')" :direction="$sorts['company_name'] ?? null">Item</x-table.heading>
                    <x-table.heading sortable multi-column wire:click="sortBy('account_number')" :direction="$sorts['account_number'] ?? null">Price</x-table.heading>
                    <x-table.heading sortable multi-column wire:click="sortBy('email')" :direction="$sorts['email'] ?? null">Unit</x-table.heading>
                    <x-table.heading sortable multi-column wire:click="sortBy('descrition')" :direction="$sorts['descrition'] ?? null">Description</x-table.heading>
                ...
                <x-slot name="body">
                    ...
                    <x-table.row class="bg-cool-gray-200" wire:key="row-message">
                        <x-table.cell colspan="6">
                            @unless ($selectAll)
                            <div>
                                <span>You selected <strong>{{ $items->count() }}</strong> items, do you want to select all <strong>{{ $items->total() }}</strong> items?</span>
                                <x-button.link wire:click="selectAll" class="ml-1 text-blue-600">Select all</x-button.link>
                            </div>
                            @else
                            You are currently selecting all <strong>{{ $items->total() }}</strong> items
                            @endif
                        </x-table.cell>
                    </x-table.row>
                    @endif
                    ...
                    <x-table.row
                        class="{{ $item->id % 2 === 0 ? 'bg-white' : 'bg-cool-gray-50' }}"
                        ...
                        <x-table.cell>
                                ...
                                wire:click="edit({{ $item->id }})">Edit</x-button.link>
                        </x-table.cell>
    ...
    <form wire:submit.prevent="deleteSelected" action="">
        <x-modal.confirmation wire:model.defer="showDeleteModal">
            <x-slot name="title">Delete items</x-slot>
